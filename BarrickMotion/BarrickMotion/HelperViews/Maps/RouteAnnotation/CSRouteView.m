...
-(void) drawRect:(CGRect) rect
{
	...
	if(!self.hidden && nil != routeAnnotation.points && routeAnnotation.points.count > 0)
	{
		...
		if(nil == routeAnnotation.lineColor)
			routeAnnotation.lineColor = [UIColor purpleColor]; // setting the property instead of the member variable will automatically reatin it.
		...
		for(int idx = 0; idx < routeAnnotation.points.count; idx++)
		{
			...
			//DebugLog(@"Point: %lf, %lf", point.x, point.y);
